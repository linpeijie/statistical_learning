# -*- coding: utf-8 -*-
"""
Created on Tue May 22 10:59:02 2018
...
def native_bayes_classifier(x_train, y_train, x_test):
    ...
    for i in range(N):
        ...
        prior_probability[i] = float(I_sum + k)/float(len(y_train) + N * k)
        for j in range(len(feature)):
            feature_class_sum = np.sum((x_train[y_train == Y[i]]) == feature[j])
            conditional_probability[i][j] = float(feature_class_sum + k)/float(I_sum + S * k)
    #计算预测数据集的后验概率
